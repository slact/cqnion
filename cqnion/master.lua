...
function Master.spawnWorker(name, chunk, ...)
  ...
  do
    ...
    if info.nups ~= nil and info.nups > 0 then
      local i, ups = 1, {}
      while i do
        local up = debug.getupvalue(chunk, i)
        if up then
  ...
  local thread, socket, thread_err = Thread.start(thread_spawner, chunk, slot, name, ...)
  if not thread then
    print(thread, socket, thread_err)
...
function Master.async(...)
  return Util.wrap(Master.controller, ...)
end
